                    ...
                    foreach ( $wpsdDonations as $donation ) {
                        ...
                        <tr>
                            ...
                            if ( wsd_fs()->is_plan('pro', true) ) {
                                ?>
                                <td><?php esc_html_e( $donation->wpsd_address ); ?></td>
                                <td><?php esc_html_e( $donation->wpsd_city ); ?></td>
                                <td><?php esc_html_e( $donation->wpsd_state ); ?></td>
                                <td><?php esc_html_e( $donation->wpsd_postcode ); ?></td>
                                <td><?php esc_html_e( $donation->wpsd_country ); ?></td>
                                <?php
                            }
                            ?>
                            <td>
...
if ( wsd_fs()->is_plan__premium_only('pro') ) {
    ?>
    <form method="post" id="wpsd-download-to-csv-form" action="">
        <input type="submit" name="wpsd_download_csv" class="button button-primary wpsd-button" value="<?php _e('EXPORT TO CSV', WPSD_TXT_DOMAIN); ?>">
    </form>
    <?php
}
?>